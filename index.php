...
$post_link = parse_url(curentUrl())['scheme'] . "://" . parse_url(curentUrl())['host'] . "/confession/" . $post->id;
...
<!--                    <li><a onclick="javascript:(document.getElementById('feed')) ? window.location.href='/confession/' + (parseInt(confessionId) + 1) : alert('Không có confession tiếp đâu!')">Tiếp</a></li>-->
                    <li><a class="btn" id="copy_to_clipboard" data-clipboard-text="<?= $post_link ?>">Copy Link</a></li>
                    <li><a class="btn" href="<?= $post_link ?>">Xem</a></li>
