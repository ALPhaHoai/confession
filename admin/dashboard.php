                ...
                <div class="row">
                    <div class="col-lg-3 col-md-6 col-sm-6">
                        <div class="card card-stats">
                            ...
                            <div class="card-footer">
                                <div class="stats">
                                    <a href="javascript: getAll('all post');">Xem hết</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-3 col-md-6 col-sm-6">
                        <div class="card card-stats">
                            ...
                            <div class="card-footer">
                                <div class="stats">
                                    <a href="javascript: getAll('not verified');">Xem hết</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12" id="result" style="display: none">
                        <div class="card">
                            ...
                            <div id="load-more-post" onclick="getAll(type)"
...
<script>
    ...
    function getAll(_type) {
        if (out_of_data && type === _type) return;
        if(type !== _type) {
            start = 0;
        ...
        type = _type;
        $.ajax({
            url: "./ajax/main.php?type=" + type + "&page=" + start,
            type: 'GET',
            dataType: 'json',
            headers: {
            ...
            complete: function (response) {
                ...
                if (response.status === 200) {
                    document.querySelector("#result .card-title").innerHTML = "All Post";
                    document.querySelector("#result .card-category").innerHTML = "Tất cả các bài viết";

                    if (response.responseText == "null" || response.responseJSON.length < 10) {
                        out_of_data = true;
                        document.getElementById("load-more-post").style.display = "none";
                        return;
                    }
    ...
    function buildPost(posts) {
        ...
        $("#result").show();
        if(start === 0) $("#table-result tbody tr").remove();

        start ++;

        for (var i = 0; i < posts.length; i++) {
            if(typeof posts[i].approval_time === "undefined") posts[i].approval_time = "null";
            if(typeof posts[i].approval_by === "undefined") posts[i].approval_by = "null";
                let str = "";
            str += "<tr id=\"" + posts[i].id + "\">" ;
            str += "<td class=\"result-id\">" + posts[i].id + "</td>" ;
            str += "<td class=\"result-content\">" + ((posts[i].content.length > 200) ? (posts[i].content.substr(0, 200) + "...") : posts[i].content) + "</td>" ;
            str += "<td class=\"result-time-approval\">" + posts[i].approval_time + "</td>";
            str += "<td class=\"result-approval-by\">" + posts[i].approval_by + "</td>";
            str += "</tr>";
